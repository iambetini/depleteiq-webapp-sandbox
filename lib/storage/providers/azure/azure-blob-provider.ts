...
import { BaseStorageProvider, StorageConfig, UploadResult } from '../../storage-provider';
...
export class AzureBlobProvider extends BaseStorageProvider {
  ...
  async upload(file: File, fileId: string, config: StorageConfig): Promise<UploadResult> {
    this.validateConfig();
    this.validateFile(file);

    try {
      // Generate unique file key
      const fileKey = generateUniqueFileKey(file.name, config.folder || this.config.folder);
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to upload file to Azure Blob Storage');
      }

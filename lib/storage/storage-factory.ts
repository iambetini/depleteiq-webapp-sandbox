...
import { AwsS3Provider, AwsS3ProxyProvider, AwsS3Config, AwsS3ProxyConfig } from './providers/aws';
...
export type StorageProviderType = 'aws-s3' | 'aws-s3-proxy' | 'azure-blob' | 'cloudinary' | 'local';
...
export class StorageFactory {
    ...
    switch (config.provider) {
      case 'aws-s3':
        return new AwsS3Provider({
          ...
          region: config.region || process.env.NEXT_PUBLIC_AWS_REGION || 'us-east-1',
        } as AwsS3Config);
      ...
      case 'azure-blob':
        return new AzureBlobProvider({
          ...baseConfig,
          accountName: config.accountName || process.env.NEXT_PUBLIC_AZURE_STORAGE_ACCOUNT_NAME || '',
          containerName: config.containerName || process.env.NEXT_PUBLIC_AZURE_STORAGE_CONTAINER_NAME || '',
          sasToken: config.sasToken || process.env.NEXT_PUBLIC_AZURE_STORAGE_SAS_TOKEN,
        } as AzureBlobConfig);

      case 'cloudinary':
        return new CloudinaryProvider({
          ...baseConfig,
          cloudName: config.cloudName || process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME || '',
          uploadPreset: config.uploadPreset || process.env.NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET || '',
        } as CloudinaryConfig);
  ...
  static getDefaultConfig(): FileUploadConfig {
    ...
    const maxFileSizeMB = parseInt(process.env.NEXT_PUBLIC_MAX_FILE_SIZE || '10');
    const maxFileSizeBytes = maxFileSizeMB * 1024 * 1024;

    return {
      provider: (process.env.NEXT_PUBLIC_STORAGE_PROVIDER as StorageProviderType) || 'aws-s3',

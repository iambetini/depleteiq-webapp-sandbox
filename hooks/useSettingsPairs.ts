...
export function useSettingsPairs(parentKey: string) {
  const { data, refetch, isFetching } = settings.useGetAllQuery({ params: { key: parentKey } });
  ...
  const handleSave = React.useCallback(async (pairs: Array<{ sub_key: string; value: string }>) => {
    await createSettings({ data: { settings: pairs.map(p => ({ key: parentKey, ...p })) } as any }).unwrap();
    toast({ title: "Success", description: `${parentKey[0].toUpperCase()}${parentKey.slice(1)} settings updated successfully` });
    refetch();

...
export default function OrdersSettingsPage() {
  ...
  const [updateSettings] = orders.useCreateMutation();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const settingsObject: OrderSettings = settings.reduce((acc, { key, value }) => {
        if (key.trim()) {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {isEditing 
              ? `Modify the order settings below. You can ${allowModifyPairs ? 'add, remove, and ' : ''}edit key-value pairs.`
              : "View current order configuration settings."
            }
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">About Order Settings</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="text-sm text-gray-600 space-y-2">
            <p>
              Order settings allow you to configure various parameters that affect how orders are processed and managed in the system.
            </p>
            <ul className="list-disc list-inside space-y-1 ml-4">
              <li>Configure order processing workflows and statuses</li>
              <li>Set up payment processing parameters</li>
              <li>Define order validation rules and constraints</li>
              <li>Manage notification preferences for order events</li>
              <li>Configure order fulfillment and shipping settings</li>
            </ul>
          </div>
        </CardContent>
      </Card>

...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!fileKey || !containerName || !accountName) {
      return createCorsResponse({ error: 'Missing required Azure configuration' }, 400);
    }
    ...
    if (!uploadResponse.ok) {
      ...
      throw new Error(`Azure Blob upload failed: ${uploadResponse.statusText} - ${errorText}`);
    }
    ...
    return createCorsResponse({ error: 'Failed to upload file to Azure Blob Storage' }, 500);
  }
}
